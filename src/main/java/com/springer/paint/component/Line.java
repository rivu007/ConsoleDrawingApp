...
public class Line extends ShapeDecorator {
    ...
    public Line(Shape canvas, int... args) {
        ...
        if(y1 < 1 || y2 < 0 || y1 > getCanvas().getHEIGHT() || y2 > getCanvas().getHEIGHT()){
            throw new ValidationException("Validation Exception: y1:"+ y1 + ", y2:"+ y2 + " co-ordinates Lshould be greater than 1 and less than canvas height: " + getCanvas().getHEIGHT());
        }
    ...
    public void draw() {
        ...
        if (x1==x2) {
            ...
            for(int col = x1; col <= x2; col++) {
                getCanvas().getCells()[y1][col] = CellContent.CROSS.symbol;
            }
        } else {
